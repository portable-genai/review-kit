...
def test_the_imported_version_is_the_declared_version() -> None:
    ...
    assert review_kit.__version__ == declared, (
        f"__init__.py exports __version__ = {review_kit.__version__!r} while "
        f"pyproject.toml declares {declared!r}. A consumer proving its adoption by printing the "
        "imported __version__ would read the wrong release, and the lockfile commit pin cannot "
        "show the difference."
    )
...
def test_the_declared_version_is_a_release_number() -> None:
    ...
    assert len(parts) == 3 and all(part.isdigit() for part in parts), (
        f"pyproject.toml declares version {_declared_version()!r}, which the lockfile header tag "
        "map and the installed-pin proof both parse as vMAJOR.MINOR.PATCH."
    )
